...
-- 插入平今次数
update times_daily set today_times=(select count(distinct capital_account_new,tradedate,done_no)
from pingcang where capital_account_new=account and vari_code=code and tradedate=date and tradedate=open_date);

-- 插入all品种
insert into times_daily (account,code,date,times,open_times,drop_times,long_times,short_times,today_times)
select account,'all',date,sum(times),sum(open_times),sum(drop_times),sum(long_times),sum(short_times),sum(today_times)
